...
def playmusic():
    ...
    ProgressbarVolumeLabel['text'] = '60%'
    mixer.music.play()
    AudioStatusLabel.configure(text='playing......')
...
img =Image.open('finalbg.jpg')
bg = ImageTk.PhotoImage(img)
label = Label(root, image=bg)
label.place(x = 0,y = 0)

root.title('Rv Music player')
root.iconbitmap(r'C:\Users\Raviraj\Downloads/pic.ico')
...
##############################################################################   Create Slider
ss = ' RV MusicPlayer '
SliderLabel = Label(root,text=ss,bg='yellow',font=('arial',25,'italic bold'))
SliderLabel.grid(row=4,column=0,padx=160,pady=160,columnspan=3)
def IntroLabelTRick():
    global count,text
    if(count>=len(ss)):
        count = -1
